...
class TelegramForwarder:
    ...
    async def list_chats(self):
        ...
        if not await self.client.is_user_authorized():
            ...
            try:
                await self.client.sign_in(self.phone_number, prompt(gradient_text('Enter the code: ', PROMPT_COLOR_START, PROMPT_COLOR_END)))
            except SessionPasswordNeededError:
                await self.client.sign_in(password=prompt(gradient_text('Two step verification is enabled. Please enter your password: ', PROMPT_COLOR_START, PROMPT_COLOR_END)))
...
""", MAIN_COLOR_START, MAIN_COLOR_END)
    ...
    if api_id is None or api_hash is None or phone_number is None:
        api_id = prompt(gradient_text("Please enter your API ID: ", PROMPT_COLOR_START, PROMPT_COLOR_END))
        api_hash = prompt(gradient_text("Please enter your API Hash: ", PROMPT_COLOR_START, PROMPT_COLOR_END))
        phone_number = prompt(gradient_text("Please enter your phone number (e.g., 447123456789): ", PROMPT_COLOR_START, PROMPT_COLOR_END))
    ...
    if profiles:
        ...
        choice = prompt(gradient_text("Do you want to use a profile? (y/n): ", PROMPT_COLOR_START, PROMPT_COLOR_END))
        if choice.lower() == 'y':
            profile_idx = int(prompt(gradient_text("Enter the profile number: ", PROMPT_COLOR_START, PROMPT_COLOR_END))) - 1
            ...
            source_chat_ids = prompt(gradient_text("Enter the source chat IDs (comma separated): ", PROMPT_COLOR_START, PROMPT_COLOR_END)).split(',')
            source_chat_ids = [int(chat_id.strip()) for chat_id in source_chat_ids]
            destination_channel_ids = prompt(gradient_text("Enter the destination chat IDs (comma separated): ", PROMPT_COLOR_START, PROMPT_COLOR_END)).split(',')
            destination_channel_ids = [int(chat_id.strip()) for chat_id in destination_channel_ids]
            keywords = prompt(gradient_text("Enter keywords to filter messages (optional). Leave blank to forward all messages: ", PROMPT_COLOR_START, PROMPT_COLOR_END)).split(',')
            keywords = [keyword.strip() for keyword in keywords if keyword.strip()]
            signature = prompt(gradient_text("Enter the signature to append to each message: ", PROMPT_COLOR_START, PROMPT_COLOR_END))
            blacklist = prompt(gradient_text("Enter blacklisted words (comma separated, or leave blank): ", PROMPT_COLOR_START, PROMPT_COLOR_END)).split(',')
            blacklist = [word.strip().lower() for word in blacklist if word.strip()]
            save_choice = prompt(gradient_text("Do you want to save this configuration as a profile? (y/n): ", PROMPT_COLOR_START, PROMPT_COLOR_END))
            if save_choice.lower() == 'y':
                profile_name = prompt(gradient_text("Enter a name for this profile: ", PROMPT_COLOR_START, PROMPT_COLOR_END))
    ...
    choice = prompt(gradient_text("Pick an option (1 or 2): ", PROMPT_COLOR_START, PROMPT_COLOR_END))
